# Health Connect Analytics Query Skill — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a `health-connect-analytics-query` skill that teaches AI agents how to authenticate to and call the webhook service's analytics routes (`/analytics/overview`, `/analytics/timeseries`, `/analytics/events`, `/analytics/export.csv`, `/healthz`).

**Architecture:** A single `SKILL.md` file in `skills/health-connect-analytics-query/` — a reference guide covering auth, endpoint table, record types, and example calls. No code changes to the app itself.

**Tech Stack:** Markdown skill file only. No dependencies.

---

## File Structure

- Create: `skills/health-connect-analytics-query/SKILL.md` — the skill reference document
...
**Files:**
- Create: `skills/health-connect-analytics-query/SKILL.md`
...
```json
{
  ...
  "stat": "sum",
  "points": [
    {"bucket_start": 1718841600000, "value": 8500.0, "count": 12, "sum": 8500, "avg": 708.33, "min": 0, "max": 15000, "latest_value": 12000, "latest_at": 1718928000000}
  ]
}
```
...
Returns raw individual health events.
...
Returns the same data as `/analytics/events` but as a downloadable CSV file.

**Query parameters:** Same as `/analytics/events` (except `limit` max is 5000).
...
Returns the health status of the Convex database connection.
...
1. **Wrong `record_type` format** — Values must be lowercase with underscores (e.g., `heart_rate`). Using `heartRate` or `heart-rate` will silently return no data.

2. **Missing `Authorization` header** — Without it, the server returns `401 Missing authorization header or dashboard session`.

3. **Confusing `INGEST_TOKEN` with `QUERY_TOKEN`** — There is no separate query token. Use the same `INGEST_TOKEN` that authenticates ingest requests.

4. **`from_ms` / `to_ms` defaults** — If not provided, endpoints use a default time window (typically the last 7–30 days depending on the endpoint). Always set explicit time windows for deterministic results.
...
6. **Timestamps are Unix milliseconds** — All time parameters and response timestamps are in Unix milliseconds, not seconds.
...
## Tooltip: Finding the Token

If you don't know the `INGEST_TOKEN` value, check the service's `.env` file:

```bash
grep INGEST_TOKEN .env
```

Or ask the service operator to provide it securely.
```

- [ ] **Step 2: Verify the file was created correctly**

Run: `cat skills/health-connect-analytics-query/SKILL.md | wc -l`
Expected: A file with substantial content (target ~250+ lines)

- [ ] **Step 3: Commit**

```bash
git add skills/health-connect-analytics-query/SKILL.md
git commit -m "docs: add health-connect-analytics-query skill for AI agents"
```
...
1. **Spec coverage:** All endpoints (`/analytics/overview`, `/analytics/timeseries`, `/analytics/events`, `/analytics/export.csv`, `/healthz`) are documented with params, example requests, and example responses. Auth is covered. Record types table is included. Common mistakes section covers all known pitfalls.
2. **Placeholder scan:** No "TBD", "TODO", or vague language. All example curl commands show real paths and params.
3. **Type consistency:** All field names in examples match the Pydantic schemas (`record_type`, `from_ms`, `to_ms`, `device_id`, etc.).

**Plan complete and saved to `docs/superpowers/plans/2026-04-20-health-connect-analytics-query-skill.md`.**
